...
[打开余薪网页版](https://wnma3mz.github.io/yuxin/)
...
- 🏁 退休倒计时支持“当前年龄 + 性别”自动估算或手动填写剩余年数，按年、月、天三种平行口径展示。
- 💰 存款只负责显示实时余额；独立的“目标计划”可选择躺平月支出或心愿物品。躺平目标用存款衡量，心愿目标则从设置当天开始随实时工资每秒推进。
- 完整存款面板会在标题中按“撑到退休每天可花”显示一条仅供娱乐的生活体感彩蛋；隐私模式和紧凑布局自动隐藏。
- 按 `v` 在本地数据、固定合成演示和计算详情之间循环，演示画面可安全截图。
- 可从配置菜单或 `yuxin share` 本地预览待提交区间，输入 `SHARE` 后才会联网贡献。
- 按 `p` 在关闭、隐藏金额和存款、再同时隐藏退休三个档位间循环；顶部口号可自定义。
- 配置支持导出、校验后导入和确认后清除，敏感数据始终保存在本机。
- 自动适配宽屏、窄屏和低高度终端；管道输出自动切换为单次快照。

默认值是月薪 8,000 元、每月 22 个工作日、周一至周五 09:00–18:00、午休 12:00–13:00、每秒刷新。薪资可切换为月薪、日薪、时薪或年薪，切换时默认金额会等价换算并取整。退休倒计时、存款和目标计划默认关闭；选择躺平目标后，“目标每月可花”默认为 3,000 元。
...
- 可输入当前年龄并选择性别自动估算，也可直接填写距离预计退休的剩余年数；输入 `0` 关闭
- 午休只填分钟数，输入 `0` 不扣除
- 存款输入 `0` 关闭；目标计划可单独开启或关闭
...
yuxin config          分模块修改配置，也可预览并匿名贡献
...
yuxin share           本地预览区间并确认后匿名贡献到公开看板
...
所有配置都保存在本机。Yuxin 不连接银行、企业系统或其他账户，安装完成后的常规仪表盘不会调用外部 API。只有用户主动运行 `yuxin update` 时会访问 GitHub；主动运行 `yuxin share`、检查待上传字段并输入 `SHARE` 后，才会向公开看板的 Supabase 服务提交降精度数据。
...
网页版首次贡献时会在当前浏览器保存随机编辑凭证，以后只更新原数值样本。数据库只保存凭证摘要；不提供账号、个人空间或凭证找回。CLI 不保存该凭证，每次确认都是新的一次性贡献。

```text
今日入账 = 当日有效工作秒数 × 每秒收入
自动退休日期 = 估算出生日期 + 按性别估算的渐进式退休年龄
手动退休日期 = 设置日期 + 用户填写的剩余年数
...
手动填写剩余退休年数时，程序不推断年龄或过去人生阶段；退休进度从设置当天的 `0%` 按剩余期限向前推进。
