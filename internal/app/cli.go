...
func runAt(args []string, stdin, stdout, stderr *os.File, now time.Time) int {
	...
	if opts.command == "share" && !opts.shareReal && !opts.shareAnonymous {
	...
	if opts.command == "share" {
		if opts.shareAnonymous {
			if !isTerminal(stdin) || !isTerminal(stdout) {
				fmt.Fprintln(stderr, "匿名贡献失败：请在交互式终端中运行 yuxin share --anonymous。")
				return 1
			}
			if err := runAnonymousShare(stdin, stdout, config, now); err != nil {
				fmt.Fprintf(stderr, "匿名贡献失败：%v\n", err)
				return 1
			}
			return 0
		}
...
func parseArgs(args []string) (cliOptions, error) {
	...
	for index := 0; index < len(args); index++ {
		...
		case opts.command == "share" && arg == "--real":
			opts.shareReal = true
		case opts.command == "share" && arg == "--anonymous":
			opts.shareAnonymous = true
	...
	if opts.shareAnonymous && (opts.shareReal || opts.shareCardSet) {
		return opts, errors.New("--anonymous 不能与 --real 或 --card 同时使用")
	}
	if opts.shareCard == "" {
...
命令：
  ...
  share --anonymous   预览并匿名贡献一份数据到公开看板
