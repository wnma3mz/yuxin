...
func loadConfig(path string) (Config, error) {
	...
	if sawProfile && (!sawProfileBirth || !sawProfileSex) {
		return config, fmt.Errorf("profile 必须同时提供 birth_date 和 sex")
	}
	return config, validateConfig(config)
}
...
func createDefaultConfig(path string) error {
	...
	if _, err := file.Write(bundledDefaultConfig); err != nil {
		file.Close()
		return err
	}
	return file.Close()
...
func validateConfig(config Config) error {
	...
	if config.SalaryAmount <= 0 {
		return fmt.Errorf("薪资金额必须大于 0")
	}
	if config.MonthlyWorkdays <= 0 {
		return fmt.Errorf("每月工作天数必须大于 0")
	}
