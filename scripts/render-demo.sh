...
mkdir -p "$OUTPUT" "$WORK"
cd "$ROOT"
go run scripts/generate-demo-data.go "$WORK/data.js"

PAGE="file://$ROOT/scripts/demo-render.html"
...
ffmpeg -hide_banner -loglevel error -y \
  ...
  "$OUTPUT/yuxin-demo.mp4"

ffmpeg -hide_banner -loglevel error -y -i "$OUTPUT/yuxin-demo.mp4" \
  ...
  -loop 0 "$OUTPUT/yuxin-demo.gif"

echo "已生成："
echo "  $OUTPUT/yuxin-demo.mp4"
echo "  $OUTPUT/yuxin-demo.gif"
