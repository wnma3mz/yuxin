...
WORK=${TMPDIR:-/tmp}/yuxin-demo-asciinema

command -v go >/dev/null 2>&1 || { echo "需要 Go" >&2; exit 1; }
command -v asciinema >/dev/null 2>&1 || { echo "需要 asciinema：brew install asciinema" >&2; exit 1; }
...
asciinema record --quiet --headless --overwrite --return \
  --window-size 110x30 --idle-time-limit 5 \
  --command "$WORK/session" "$WORK/yuxin-demo.cast"

agg --quiet --theme github-dark --font-size 16 --line-height 1.35 \
  --fps-cap 12 --idle-time-limit 5 --last-frame-duration 2 --select event:0..100% \
