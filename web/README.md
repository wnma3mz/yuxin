...
> 状态：v0.2.1 已完成本地开发与验证，待推送后部署；匿名纠错 migration 和旧 CLI 向后兼容探测已完成。
...
这个目录用于独立的 GitHub Pages 站点。访客无需登录即可在当前浏览器计算自己的今日入账和下班倒计时，也可以主动匿名贡献一组 Yuxin 数据并查看公开聚合看板。网页不提供账号、服务端个人档案或跨设备同步，也不会自动读取本机的 `yuxin.toml` 或 `~/.config/yuxin/config.toml`。

## MVP 边界

页面提供三个彼此隔离的状态：

1. **我的余薪**：首屏先展示明确标记的动态演示数据；用户配置后改为读取当前浏览器的本地数据，不联网保存。
2. **公开看板**：所有访客均可查看样本量、中位数和区间分布。
3. **匿名贡献**：直接复用本地配置，但只有用户选择字段并最终确认后才提交。
...
- **服务端个人空间**：不提供个人主页、云端个人档案、投稿历史、找回能力或跨设备同步；首屏本地看板只读取当前浏览器的站点存储。
...
- `submit_public_data`：校验字段范围；无凭证时新增一条匿名贡献，凭证摘要匹配时只更新原数值样本。
...
网页首次贡献时会生成 256 位随机编辑凭证，原文只保存在当前浏览器，数据库只保存 SHA-256 摘要。同一浏览器后续贡献只更新原数值样本，不新增一份；不提供记录列表、账号、个人空间或凭证找回。清除站点数据后仍无法修改服务端的旧样本。CLI 不保存编辑凭证，每次确认都是新的一次性贡献。

### 匿名性的准确边界

- Yuxin 应用数据不保存姓名、联系方式、IP、User-Agent、设备 ID 或精确提交时间。网页纠错只额外保存一个与单条数值样本绑定的随机凭证摘要，不用于跨记录识别或用户画像。
...
- 文本提交后默认为 `pending`；数值提交后立即进入公开聚合，每新增一份样本都会重新计算。公开总数按实际样本数展示，分桶和有效样本数按 5 份向下分组，中位数再按固定粒度取整。正文需在 Supabase 后台标记为 `approved` 才会公开。
...
- 首屏动态演示卡会让工资持续跳动，“下一个盼头”始终按当天和内置节假日日历计算；用户保存配置后原地切换为本地实时卡，显示今日入账、下班状态、节假日、退休与躺平预算
- 本地配置通过版本化 `localStorage` 保存，不生成设备指纹、不自动上传；退休可按年龄与性别估算，也可手动填写剩余年数
- 保存配置后可在“我的数据”和“演示数据”之间临时切换；演示预览不删除配置，刷新页面默认回到我的数据
- 随机编辑凭证与本地配置分开保存；清除本地计算配置不会主动丢失纠错能力，清除整个站点数据才会删除凭证
- 匿名贡献不再重复填写工资和工时；存款与退休字段默认不参与，必须由用户单独勾选并二次确认
...
- 宏观指标集中展示公开样本量、月薪、工时和折算时薪中位数；象限和区间只显示固定趣味旁白，不冒充从用户回声提取的高频词。
